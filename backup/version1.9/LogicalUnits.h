...
struct Needregids_ {
	...
	void Proc(int f_icode) {
		val = set<int> { IRRMOVL, IOPL, ICMPL, IIROPL, IPUSHL, IPOPL, IIRMOVL, IRMMOVL, IMRMOVL }.count(f_icode);
	}
};
...
struct NeedvalC_ {
	...
	int Get() { return val; }
	void Proc(int f_icode) {
		val = set<int>{ IIRMOVL, IRMMOVL, IIROPL, IMRMOVL, IJXX, ICALL }.count(f_icode);
	}
};
/*
HCL:
...
struct Instrvalid_ {
	...
	void Proc(int f_icode) {
		val = set<int> { 
			INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  	    IOPL, ICMPL, IIROPL, IJXX, ICALL, IRET, IPUSHL, IPOPL
		}.count(f_icode);		
	}
};
...
struct d_srcB_ {
	...
	int Get() { return val; }
	void Proc(int D_icode, int D_rB) {
		if (set<int>{ IOPL, ICMPL, IIROPL, IRMMOVL, IMRMOVL }.count(D_icode)) val = D_rB;
		else if (set<int> { IPUSHL, IPOPL, ICALL, IRET }.count(D_icode)) val = RESP;
		else val = RNONE;
	}
};
...
struct ALUA_ {
	...
	void Proc(int E_icode, int E_valC, int E_valA) {
		...
		else if (set<int>{ IIRMOVL, IRMMOVL, IMRMOVL, IIROPL }.count(E_icode)) val = E_valC;
...
struct ALUB_ {
	...
	void Proc(int E_icode, int E_valB) {
		if (set<int>{ IRMMOVL, IMRMOVL, IIROPL, IOPL, ICMPL, ICALL, 
...
struct ALUfun_ {
	...
	void Proc(int E_icode, int E_ifun) {
		if (set<int> { IOPL, IIROPL }.count(E_icode))
			val = E_ifun;
		else if (set<int> { ICMPL }.count(E_icode))
			val = ALUSUB;
		else val = ALUADD;
	}
};
...
struct SetCC_ {
	...
	void Proc(int E_icode, int W_stat, int m_stat) {
		val = (set<int> { IOPL, IIROPL, ICMPL }.count(E_icode) && 

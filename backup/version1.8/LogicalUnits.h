...
struct Needregids_ {
	...
	void Proc(int f_icode) {
		val = set<int> { IRRMOVL, IOPL, ICMPL, IPUSHL, IPOPL, IIRMOVL, IRMMOVL, IMRMOVL }.count(f_icode);
	}
};
...
struct Instrvalid_ {
	...
	void Proc(int f_icode) {
		val = set<int> { 
			INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  	    IOPL, ICMPL, IJXX, ICALL, IRET, IPUSHL, IPOPL
		}.count(f_icode);		
	}
};
...
struct d_srcB_ {
	...
	int Get() { return val; }
	void Proc(int D_icode, int D_rB) {
		if (set<int>{ IOPL, ICMPL, IRMMOVL, IMRMOVL }.count(D_icode)) val = D_rB;
		else if (set<int> { IPUSHL, IPOPL, ICALL, IRET }.count(D_icode)) val = RESP;
		else val = RNONE;
	}
};
...
struct ALUB_ {
	...
	void Proc(int E_icode, int E_valB) {
		if (set<int>{ IRMMOVL, IMRMOVL, IOPL, ICMPL, ICALL, 
...
struct SetCC_ {
	...
	void Proc(int E_icode, int W_stat, int m_stat) {
		val = (set<int> { IOPL, ICMPL }.count(E_icode) && 
